...
from model.TransformerPlus import Transformer
...
class OneModel(nn.Module):
    ...
    def forward(self, x, y_m=None, y_b=None, s_x=None, s_y=None, cat_idx=None):
        ...
        # Modifications:
        # 1. torch.no_grad()
        # 2. when extracting support features, do not use mask
        # ========================================
        ...
        mask = (mask == 1).float()
        supp_mask = F.interpolate(mask, size=(supp_feat_3.size(2), supp_feat_3.size(3)), mode='bilinear', align_corners=True)  # (bs*shot, 1, h, w)
        ...
        # Prior masks
        # Modifications:
        # 1. replace the original one as our prior generator
        # ========================================
        fts_size = query_feat.size()[-2:]
        if self.shot == 1:
            corr_fg_4, _, corr_4, corr_fg_4_sim_max, corr_4_sim_max = self.generate_prior_proto(query_feat_4, [supp_feat_4], [supp_mask], fts_size)
            corr_fg_5, _, corr_5, corr_fg_5_sim_max, corr_5_sim_max = self.generate_prior_proto(query_feat_5, [supp_feat_5], [supp_mask], fts_size)
        else:
            ...
            # supp_mask = rearrange(supp_mask, "b n c h w -> (b n) c h w")
