...
from model.cyc_transformer_plus import CyCTransformerPlus
...
class OneModel(nn.Module):
    def __init__(self, args):
        ...
        self.transformer = CyCTransformerPlus(embed_dims=reduce_dim, shot=self.shot, num_points=9)
        ...
        # use predicted query to predict support
        qry_dim_scalar = 1
        ...
        PSPNet_ = PSPNet(args)
        pre_weight = "../HDMNet/initmodel/PSPNet/{}/split{}/{}/best.pth".format(self.dataset, args.split, "resnet50" if not args.vgg else "vgg")
        print(pre_weight)
    ...
    def get_optim(self, model, args, LR):
        optimizer = torch.optim.SGD(
            ...
            ], lr=LR, momentum=args.momentum, weight_decay=1e-4
        )
    ...
    def generate_prior(self, query_feat_high, final_supp_list, mask_list, fts_size):
        ...
        for i, tmp_supp_feat in enumerate(final_supp_list):
            ...
            sim_max = similarity.max(1)[0]  # bsize
        ...
        return corr_query_mask, sim_max
    ...
    def generate_prior_pfenet(self, query_feat_high, final_supp_list, mask_list, fts_size):
        bsize, ch_sz, sp_sz, _ = query_feat_high.size()[:]
        corr_query_mask_list = []
        cosine_eps = 1e-7
        for i, tmp_supp_feat in enumerate(final_supp_list):
    ...
    def forward(self, x, s_x, s_y, y_m, padding_mask=None, s_padding_mask=None, cat_idx=None):
        ...
        supp_bcb_fts['3'] = F.interpolate(supp_bcb_fts['3'], size=fts_size, mode='bilinear', align_corners=True)
        supp_feat_high_4 = supp_bcb_fts['3'].view(bs, self.shot, -1, fts_size[0], fts_size[1])
        supp_bcb_fts['4'] = F.interpolate(supp_bcb_fts['4'], size=fts_size, mode='bilinear', align_corners=True)
        ...
        supp_mask = F.interpolate((s_y == 1).view(-1, *x_size[-2:]).float().unsqueeze(1), size=fts_size, mode='bilinear', align_corners=True)  # bs*shot, 1, 60, 60
        if self.shot > 1:
            multi_supp_pro = Weighted_GAP(supp_feat, supp_mask).expand(-1, -1, fts_size[0], fts_size[1])  # bs*shot, c, 60, 60
        else:
            multi_supp_pro = supp_pro
        ...
        supp_cat = torch.cat([supp_feat, multi_supp_pro], dim=1)  # bs, 512, 60, 60
        aug_supp_feat = self.init_merge_supp(supp_cat)  # bs, 256, 60, 60

        # Transformer
        query_feat_list, pseudo_masks = self.transformer(query_feat, padding_mask.float(), aug_supp_feat, s_y.clone().float(), s_padding_mask.float())
        fused_query_feat = []
        for idx, qry_feat in enumerate(query_feat_list):
            fused_query_feat.append(
                F.interpolate(qry_feat, size=fts_size, mode='bilinear', align_corners=True)
            )
        ...
        # Meta Output
        # ========================================
        query_meta = self.res1_meta(merge_feat)
        ...
        # Base Output
        # ========================================
        base_out = F.interpolate(qry_bcb_fts['4'], size=fts_size, mode='bilinear', align_corners=True)
        ...
        if self.training:
            ...
            # for discriminative adaptor
            for pseudo_mask in pseudo_masks:
                gt_mask = (y_m == 1).float().unsqueeze(1)
            ...
            qry_mask = F.interpolate((y_m == 1).float().unsqueeze(1), size=query_meta.size()[-2:], mode='bilinear', align_corners=True)
            qry_proj_feat = self.pred_supp_qry_proj(query_meta) + mid_query_feat
            ...
            for st_id, supp_out in enumerate(supp_out_list):
                ...
                supp_loss = self.criterion_dice(supp_out, s_y[:, st_id, ...].long())
                aux_loss3 += supp_loss / self.shot
