...
class CrossWindowAttention(nn.Module):
    ...
    def forward(self, q, s, q_mask=None, s_mask=None):
        ...
        if s_mask is not None:
            mask = s_mask.unsqueeze(1).transpose(-2, -1).clone()  # B_, 1, 1, N
            mask[mask < 0.5] = -100.
            mask[mask >= 0.5] = 0.
            attn_cross += mask
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
        ...
        # Discriminative Adaptor
        self.disc = DiscAdaptor(dim, drop=drop)
    ...
    def forward(self, q, s, s_mask):
        ...
        # Discriminative Adaptor
        # ========================================
        q, s, pseudo_mask = self.disc(q, s, (H, W), s_mask)
        ...
        attn_q_windows = self.attn_q(q_windows, qs_windows, s_mask=s_mask_windows)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, q, s, s_mask, H, W):
        ...
        for blk in self.blocks:
            blk.H, blk.W = H, W
            q, s, pseudo_mask = blk(q, s, s_mask)
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, q, s, s_mask):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            q, s, pseudo_mask = layer(q, s, s_mask, Wh, Ww)
